...
const CONFIG = {
  ...
  BASE_URL_WITH_FORCED: (forcedValue) =>
    `https://yesno.wtf/api?force=${forcedValue}`,
};

window.addEventListener('DOMContentLoaded', () => {
  const form = document.querySelector('#questionForm');
  const forcedAnswerCheckbox = document.querySelector('#forcedAnswerCheckbox');
  const forcedAnswerContainer = document.querySelector(
    '#forcedAnswerContainer'
  );
  const forcedAnswerRadios = document.querySelectorAll(
    'input[name="forcedAnswer"]'
  );
